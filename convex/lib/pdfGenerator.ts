...
export function htmlToBlocks(html: string): Array<Block> {
  ...
  // Collect all tokens
  const tokens: Array<{ tag: string; closing: boolean; index: number; length: number }> = [];
...
class PdfWriter {
  ...
  private drawLine(
    ...
    for (const run of runs) {
      if (run.text) {
        this.page.drawText(run.text, { x: cx, y: this.y, size, font: run.font, color });
  ...
  drawText(text: string, bold: boolean, size: number, lineHeight: number, color: RGB, indent = 0) {
...
export async function generatePdf(
  ...
  w.gap(4);
  w.drawText(`Generated on ${new Date().toLocaleDateString()}`, false, 9, 12, MUTED);
  w.gap(8);
...
export function htmlToSections(
  ...
  const flush = () => {
    if (currentHeading || currentBody.trim()) {
      sections.push({ heading: currentHeading || "Overview", body: currentBody.trim() });
    }
    currentBody = "";
  };
  ...
  flush();
  return sections.length > 0 ? sections : [{ heading: "Report", body: stripTags(html).trim() }];
}
