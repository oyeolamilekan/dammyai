...
export function createCheckScheduleTool(ctx: AILikeCtx, userId: string) {
  return tool({
    ...
    inputSchema: z.object({
      ...
      daysAhead: z.number().optional().describe("Number of days ahead to look (default 1, max 30)"),
      maxResults: z.number().optional().describe("Maximum events to return (default 10, max 25)"),
    ...
    execute: async ({ date, daysAhead, maxResults, query }) => {
      ...
      return events
        .map((e) => {
          ...
          if (e.description) parts.push(`   Notes: ${e.description.slice(0, 100)}`);
          return parts.join("\n");
        })
        .join("\n\n");
    },
...
export function createScheduleCallTool(ctx: AILikeCtx, userId: string) {
  return tool({
    description:
      "Schedule a call or meeting on Google Calendar.",
    inputSchema: z.object({
      participant: z.string().describe("Name or email of the person to schedule with"),
    ...
    execute: async ({ participant, date, time, durationMinutes, topic }) => {
      ...
      const event = {
        summary: topic ? `${topic} with ${participant}` : `Call with ${participant}`,
        description: topic ?? undefined,
        start: { dateTime: startDateTime.toISOString() },
        end: { dateTime: endDateTime.toISOString() },
        attendees: participant.includes("@") ? [{ email: participant }] : undefined,
      };
...
export function createRemoveEventTool(ctx: AILikeCtx, userId: string) {
  return tool({
    ...
    inputSchema: z.object({
      ...
      daysAhead: z.number().optional().describe("Days ahead to search (default 7, max 30)"),
    }),
